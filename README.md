# 🛒 **Retail Sales Performance, Customer Segmentation & Discount Optimization Dashboard**

---

## 📌 **Overview**

This is an end-to-end **data analytics project** focused on analyzing and optimizing retail sales performance. The project leverages **SQL** for data processing and advanced analytics, and **Power BI** for interactive visualization, enabling **data-driven decision-making** across revenue, customer behavior, and pricing strategies.

---

## 🚀 **Features**

* **Interactive Power BI Dashboard** – KPI-driven insights for stakeholders
* **Sales Performance Analysis** – Revenue, monthly trends, and order metrics
* **Customer Segmentation** – RFM-based segmentation and Customer Lifetime Value (CLV)
* **Discount Optimization** – Impact of discounts on revenue and pricing strategy
* **Product & Category Insights** – Identification of top-performing products
* **Geographic Analysis** – Store-level and city-level performance tracking

---

## 📂 **Data Sources**

* **Customers** – Demographics, signup date, behavioral attributes
* **Products** – Product details, categories, pricing
* **Sales** – Transaction-level purchase data and discounts
* **Stores** – Store location and type

---

## ⚙️ **Tech Stack**

* **SQL** – Data cleaning, transformation, KPI calculations, advanced analytics
* **Power BI** – Dashboard development, KPI visualization, business storytelling

---

## 🔄 **Workflow**

### **1. Data Preparation (SQL)**

* Cleaned and transformed raw data
* Converted discount fields into numeric format
* Created derived metrics (gross revenue, net sales)

### **2. Data Analysis (SQL)**

* Calculated KPIs: **Revenue, AOV, CLV**
* Performed **RFM-based customer segmentation**
* Implemented advanced SQL techniques:

  * Window Functions (**RANK(), LAG(), Running Totals**)
  * CTEs for modular queries
* Conducted discount impact and customer behavior analysis

### **3. Visualization (Power BI)**

* Built an interactive dashboard with:

  * Revenue and sales trends
  * Customer and product insights
  * Discount impact analysis
  * Geographic performance

---

## 📊 **Key Insights**

* **No Discount** generates the highest revenue; discounted buckets contribute less, and no clear evidence shows 0–5% as optimal.
* **High-value customers** contribute a major share of revenue
* **Top product categories** drive the majority of sales
* **Regional performance differences** highlight growth opportunities

---

## 📈 **Results**

* Delivered actionable insights for **sales optimization and pricing strategy**
* Identified **high-value customers and top-performing products**
* Enabled **data-driven decision-making** through an interactive dashboard

---

## 👩‍💻 **Author**

**Pooja Dhamale**
*Aspiring Data Analyst*
**Skills:** SQL | Power BI | Data Visualization

---
